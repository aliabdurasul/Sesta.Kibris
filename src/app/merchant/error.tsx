...
/**
 * Merchant route error boundary.
 * Catches unexpected render errors in /merchant/* without showing the root error page.
 */
...
export default function MerchantError({ error, reset }: Props) {
  useEffect(() => {
    if (process.env.NODE_ENV !== "production") {
      console.error("[MerchantError]", error.message);
    }
  }, [error]);
    ...
    <div className="flex min-h-[60vh] flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm rounded-2xl bg-white p-8 text-center shadow-sm ring-1 ring-gray-100">
        ...
        <p className="mt-2 text-sm text-gray-500">
          Market paneli yüklenirken beklenmedik bir hata meydana geldi.
        </p>
